...
- [ ] **DATA-01**: raw/standard/featureの3層スキーマを定義し、保存形式・カラム名・データ型を文書化できること
...
- [ ] **DATA-04**: データリーク防止のため、事前/事後カラムの監査機構があり、レース後に確定する情報が特徴量に混入しないことを検証できること
- [ ] **DATA-05**: 2015-2024年のデータ（Kaggle + 自前収集）を共通standard形式でParquet出力し、統合して扱えること

### Scraping

- [ ] **SCRP-01**: fetch/parse/normalize/featureの処理を分離したスクレイピング基盤を構築できること
- [ ] **SCRP-02**: 2022年以降のJRAレース結果・出走馬情報のHTMLを取得しraw保存できること
...
- [ ] **MODA-01**: LightGBMによる3着内確率モデルを構築し、各馬のp_top3を出力できること
- [ ] **MODA-02**: TimeSeriesSplitによる時系列CVで学習/検証を分離し、未来データのリークを防ぐこと
